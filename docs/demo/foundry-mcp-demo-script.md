...
- [ ] ServiceNow instance credentials ready (instance URL + username/password)
...
💬 *"Credentials live in a single JSON file. Supports multiple named profiles — dev, staging, customer instances."*
...
- [ ] `~/.servicenow/credentials.json` created with at least one profile
...
```
foundry_init projectName="my-poc" path="~/projects/my-poc" template="foundry-poc"
```
...
💬 *"Background scripts for anything the REST API can't do — version checks, complex lookups, on-the-fly calculations."*

```
servicenow_script script="gs.info('Instance: ' + gs.getProperty('instance_name')); gs.info('Version: ' + gs.getProperty('glide.buildtag'));"
```

**Expected:** Returns instance name and build version from script output.

- [ ] Script executes and returns output
...
| `servicenow_connect` fails | Wrong credentials or instance URL | Check `~/.servicenow/credentials.json` |
...
*Last updated: 2026-03-10 | MCP Server version: 2026.03.0910 | 38 tools*

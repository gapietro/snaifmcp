...
export const SERVICENOW_DISCONNECT_TOOL: Tool = {
...
export const SERVICENOW_STATUS_TOOL: Tool = {
...
export const SERVICENOW_SYSLOGS_TOOL: Tool = {
...
export const SERVICENOW_AIA_LOGS_TOOL: Tool = {
...
export const SERVICENOW_QUERY_TOOL: Tool = {
...
export const SERVICENOW_SCRIPT_TOOL: Tool = {
...
export const SERVICENOW_INSTANCE_TOOL: Tool = {
...
export const SERVICENOW_TOOLS: Tool[] = [
...
export async function handleServiceNowConnect(args: Record<string, unknown>): Promise<ToolResult> {
  ...
  const result = await connectionManager.connect({
    instance,
    authType: authType as 'basic' | 'token' | 'oauth' | undefined,
    profile,
    username,
    password,
    token,
...
export async function handleServiceNowDisconnect(args: Record<string, unknown>): Promise<ToolResult> {
...
export async function handleServiceNowStatus(_args: Record<string, unknown>): Promise<ToolResult> {
  const status = connectionManager.getStatus();
...
export async function handleServiceNowSyslogs(args: Record<string, unknown>): Promise<ToolResult> {
  // Check connection
  if (!connectionManager.isConnected()) {
    return {
      content: [{
...
Example:
  ...
  // Parse arguments
  const level = (args.level as string) || 'error';
  const source = args.source as string | undefined;
  const messageFilter = args.message as string | undefined;
  const timeRange = (args.timeRange as string) || '1h';
...
Try expanding the time range or adjusting filters.`,
    ...
    const formattedLogs = logs.map((log, index) => {
      const levelNum = parseInt(log.level as string, 10);
      const levelName = SYSLOG_LEVEL_NAMES[levelNum] || `LEVEL_${levelNum}`;
      const timestamp = log.sys_created_on as string;
      const src = log.source as string || 'unknown';
      const msg = log.message as string || '';
      const user = log.sys_created_by as string || 'system';
...
const AIA_STATUS_MAP: Record<string, string> = {
  success: 'success',
  completed: 'success',
  failure: 'failure',
  failed: 'failure',
...
interface AIAExecution {
  ...
  errorMessage?: string;
  toolCalls: AIAToolExecution[];
}

export async function handleServiceNowAiaLogs(args: Record<string, unknown>): Promise<ToolResult> {
  // Check connection
  if (!connectionManager.isConnected()) {
    return {
      content: [{
...
Example:
  ...
  const timeRange = (args.timeRange as string) || '4h';
...
Try expanding the time range or adjusting filters.`,
    ...
    const toolCallsMap = new Map<string, AIAToolExecution[]>();
        ...
        try {
          ...
          if (toolResponse.result) {
            for (const tool of toolResponse.result) {
              const execId = (tool.execution as string) ||
                ((tool.execution as { value?: string })?.value);
              if (execId) {
                if (!toolCallsMap.has(execId)) {
                  toolCallsMap.set(execId, []);
                }
                toolCallsMap.get(execId)!.push({
                  toolName: (tool.tool_name as string) || 'unknown',
                  input: (tool.input as string) || '',
                  output: (tool.output as string) || '',
                  status: (tool.status as string) || 'unknown',
                  duration: parseInt(tool.duration as string, 10) || 0,
                  errorMessage: tool.error_message as string | undefined,
                  stepNumber: parseInt(tool.step_number as string, 10) || 0,
                });
              }
            }
    ...
    // Format executions
    const formattedExecutions: AIAExecution[] = executions.map(exec => {
      const execId = exec.sys_id as string;
      ...
      return {
        ...
        startTime: (exec.sys_created_on as string) || '',
        endTime: exec.sys_updated_on as string | undefined,
        duration: parseInt(exec.duration as string, 10) || undefined,
        triggerType: exec.trigger_type as string | undefined,
        triggerContext: exec.trigger_context as string | undefined,
        errorMessage: exec.error_message as string | undefined,
...
export async function handleServiceNowQuery(args: Record<string, unknown>): Promise<ToolResult> {
  // Check connection
  if (!connectionManager.isConnected()) {
    return {
      content: [{
...
Try adjusting your query or checking the table name.`,
    ...
    const formattedRecords = records.map((record, index) => {
      const lines: string[] = [`[${index + 1}] sys_id: ${record.sys_id}`];

      for (const [key, value] of Object.entries(record)) {
        if (key === 'sys_id') continue; // Already shown
        ...
        if (value === null || value === undefined) {
          ...
          const refObj = value as { display_value?: string; value?: string };
...
const MUTATION_PATTERNS: Array<{ pattern: RegExp; operation: string }> = [
...
function analyzeScript(script: string): ScriptAnalysisResult {
...
interface HealthMetrics {
  semaphores?: { available: number; max: number };
  scheduledJobs?: { running: number; queued: number };
  transactions?: { current: number };
  cacheHitRate?: number;
}

export async function handleServiceNowInstance(args: Record<string, unknown>): Promise<ToolResult> {
  // Check connection
  if (!connectionManager.isConnected()) {
    return {
      content: [{
...
Example:
  ...
  const checkFeatures = (args.checkFeatures as string[]) || ['now_assist', 'virtual_agent', 'aia'];
  ...
  try {
    ...
    let health: HealthMetrics | undefined;
    if (includeHealth) {
...
async function getInstanceInfo(client: ServiceNowClient): Promise<{
  version: string;
  buildTag?: string;
  buildDate?: string;
}> {
...
async function getHealthMetrics(client: ServiceNowClient): Promise<HealthMetrics> {
  ...
  try {
    ...
    if (response.result && response.result.length > 0) {
      ...
      for (const sem of response.result) {
        totalMax += parseInt(sem.max_count as string, 10) || 0;
        totalAvailable += parseInt(sem.count as string, 10) || 0;
      }
      if (totalMax > 0) {
  ...
  try {
    const runningResponse = await client.queryTable(
      'sys_trigger',
      'state=executing',
      ['sys_id'],
      100
    );
    const queuedResponse = await client.queryTable(
      'sys_trigger',
      'state=queued',
      ['sys_id'],
      100
...
export async function handleServiceNowTool(
  name: string,
...
export function isServiceNowTool(name: string): boolean {

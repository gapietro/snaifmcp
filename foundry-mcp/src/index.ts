...
import {
  ...
  handleSkillTool,
  isSkillTool,
} from "./servicenow/tools-skills.js";
 ...
 */
async function warnMissingRecommendedSkills(): Promise<void> {
  if (!CONFIG.notifyOnRelevantNotInstalled) return;
  ...
  try {
    ...
    for (const skill of recommended) {
      ...
      if (!installed) {
        console.error(
          `[foundry] Recommended global skill not installed: ${skill.name}\n` +
 ...
 */
async function readInstalledSkillContent(name: string): Promise<string | null> {
...
async function main() {
  ...
  server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
    ...
    if (!content) throw new Error(`Skill "${skillName}" not found or not installed globally`);
    return {
      contents: [
        {

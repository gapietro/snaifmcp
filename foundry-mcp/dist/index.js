...
const FOUNDRY_INIT_TOOL = {
    name: "foundry_init",
    description: "Bootstrap a new Now Assist POC project with pre-loaded context, skills, and SPARC template from the Foundry golden repository.",
    inputSchema: {
        type: "object",
        properties: {
            projectName: {
...
async function ensureGoldenRepo() {
    ...
    if (exists) {
        ...
        try {
            ...
            throw new Error(`Failed to clone golden repository: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
 ...
 */
async function initializeProject(projectName, parentPath, goldenPath) {
...
async function main() {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        if (name === "foundry_init") {
            ...
            const result = await initializeProject(projectName, parentPath, goldenPath);
            return {
                content: [
                    {
                        type: "text",

            ...
            switch (profile.type) {
                case 'basic':
                    if (!profile.username || !profile.password) {
                        throw new ServiceNowError(ServiceNowErrorType.AUTHENTICATION_FAILED, 'Profile missing username or password', { profile: params.profile });
                    }
                    return {
                        type: 'basic',
                ...
                case 'token':
                    if (!profile.token) {
                        throw new ServiceNowError(ServiceNowErrorType.AUTHENTICATION_FAILED, 'Profile missing token', { profile: params.profile });
                    }
                    return { type: 'token', token: profile.token };
                case 'oauth':
                    if (!profile.clientId || !profile.clientSecret) {
                        throw new ServiceNowError(ServiceNowErrorType.AUTHENTICATION_FAILED, 'Profile missing OAuth credentials', { profile: params.profile });
                    }
                    return {
                        type: 'oauth',
        ...
        switch (authType) {
            case 'basic':
                if (!params.username || !params.password) {
                    throw new ServiceNowError(ServiceNowErrorType.AUTHENTICATION_FAILED, 'Basic auth requires username and password', undefined, 'Provide username and password parameters');
                }
                return {
                    type: 'basic',
            ...
            case 'token':
                if (!params.token) {
                    throw new ServiceNowError(ServiceNowErrorType.AUTHENTICATION_FAILED, 'Token auth requires a token', undefined, 'Provide the token parameter');
                }
                return { type: 'token', token: params.token };
            case 'oauth':
                if (!params.clientId || !params.clientSecret) {
                    throw new ServiceNowError(ServiceNowErrorType.AUTHENTICATION_FAILED, 'OAuth requires clientId and clientSecret', undefined, 'Provide OAuth application credentials');
                }
                return {
                    type: 'oauth',
